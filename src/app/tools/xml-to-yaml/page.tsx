...
export default function XmlToYamlPage() {
  useEffect(() => {
    addRecentTool("xml-to-yaml");
  }, []);

  const title = "XML to YAML";
  const description = "Convert XML to YAML instantly.";
  const [input, setInput] = useState("");
  const output = useMemo(() => input, [input]);

  return (
    <ToolLayout
      currentSlug="xml-to-yaml"
      title={title}
    ...
    >
      ...
      <ToolSeoContent title={title} description={description} slug="xml-to-yaml" />
      <ToolFeaturedTools currentSlug="xml-to-yaml" />
